...
from utils.read_csv import read_from_aois_file, read_from_input_files, read_from_trial_index_file
...
def do_preprocessing(input_data_files: str = [YOUNG_ADULTS_1, YOUNG_ADULTS_2],
    ...
    if input_trial_index_file is None:
        trial_index = create_trial_id_index(df, to_file=output_trial_index_file)
    else:
        trial_index = read_from_trial_index_file(input_trial_index_file)
