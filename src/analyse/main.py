...
def descriptives_analysis(input_aoi_analysis_file: str = None):
    ...
    display(trial_id_response_df[LIE].apply(lambda x: x[0]).quantile(0.25))
    display(trial_id_response_df[LIE].apply(lambda x: x[0]).quantile(0.75))
    display(trial_id_response_df[LIE].apply(lambda x: x[0]).min())
    display(trial_id_response_df[LIE].apply(lambda x: x[0]).max())
    ...
    display(pid_response_df[LIE].apply(lambda x: x[0]).quantile(0.25))
    display(pid_response_df[LIE].apply(lambda x: x[0]).quantile(0.75))
    display(pid_response_df[LIE].apply(lambda x: x[0]).min())
    display(pid_response_df[LIE].apply(lambda x: x[0]).max())
    display(get_confidence_intervals(pid_response_df[LIE]))
    print(TRIAL)
    ...
    display(aoi_analysis_df[RT].mean())
    display(aoi_analysis_df[RT].std())
    display(aoi_analysis_df[RT].quantile(0.25))
    display(aoi_analysis_df[RT].quantile(0.75))
    ...
    print(N_TRANSITIONS)
    display(aoi_analysis_df[N_TRANSITIONS].mean())
    display(aoi_analysis_df[N_TRANSITIONS].std())
    display(aoi_analysis_df[N_TRANSITIONS].quantile(0.25))
    display(aoi_analysis_df[N_TRANSITIONS].quantile(0.75))
    gains_response_df = get_gains_response_stats(aoi_analysis_df, [GAIN_UNDER_TEN, GAIN_OF_TEN, GAIN_OF_TWENTY, GAIN_OF_THIRTY, GAIN_UNDER_THIRTY], with_error_bounds=False)
    display(gains_response_df[[SELF_LIE, SELF_TRUE, OTHER_LIE, OTHER_TRUTH, N_TRANSITIONS]], max_cols=None)
    gains_response_df = get_gains_response_stats(aoi_analysis_df, [LOSS_UNDER_TEN, LOSS_OF_TEN, LOSS_OF_TWENTY, LOSS_UNDER_THIRTY, LOSS_OF_THIRTY], with_error_bounds=False)
    display(gains_response_df[[SELF_LIE, SELF_TRUE, OTHER_LIE, OTHER_TRUTH, N_TRANSITIONS]], max_cols=None)
...
def response_analysis(input_aoi_analysis_file: str = None,
                      ...
                      avg_dwell_per_gain_30_plot: str = None,
                      avg_n_transition_per_gain_30_plot: str = None,
                      avg_dwell_per_loss_30_plot: str = None,
                      avg_n_transition_per_loss_30_plot: str = None,
                      output_trial_index_gains_plot: str = None,
                      output_trial_index_losses_plot: str = None,
                      colors: List[str] = XKCD_COLORS_LIST):
    purple, sand = XKCD_COLORS["xkcd:purple"], XKCD_COLORS["xkcd:light tan"]
    ...
    dwell_as_a_proportion = True
    aoi_analysis_df = read_from_analysis_file(input_aoi_analysis_file, dwell_as_a_proportion=dwell_as_a_proportion)
    ...
    plot_losses_by_trial_id(gains_df, colors=[XKCD_COLORS["xkcd:darkish red"]], to_file=output_trial_index_losses_plot)
    ...
    plot_gains_mean_percent_lie(aoi_analysis_df, [NEGATIVE_GAIN, GAIN_UNDER_TEN, GAIN_OF_TEN, GAIN_OF_TWENTY, GAIN_OF_THIRTY],
                                title=title, colors=[sand, XKCD_COLORS["xkcd:pinkish purple"], XKCD_COLORS["xkcd:purply"], purple, XKCD_COLORS["xkcd:darkish purple"]], to_file=net_gain_lie_plot)
    ...
    plot_gains_avg_dwell_time(aoi_analysis_df, [GAIN_UNDER_TEN, GAIN_OF_TEN], is_dwell_a_proportion=dwell_as_a_proportion, colors=[
                              XKCD_COLORS["xkcd:pinkish purple"], XKCD_COLORS["xkcd:purply"]], to_file=avg_dwell_per_gain_plot)
    plot_gains_avg_dwell_time(aoi_analysis_df, [GAIN_UNDER_THIRTY, GAIN_OF_THIRTY], is_dwell_a_proportion=dwell_as_a_proportion,
                              colors=[purple, XKCD_COLORS["xkcd:darkish purple"]], to_file=avg_dwell_per_gain_30_plot)
    plot_gains_n_transitions(aoi_analysis_df, [GAIN_UNDER_TEN, GAIN_OF_TEN], [XKCD_COLORS["xkcd:pinkish purple"], XKCD_COLORS["xkcd:purply"]], to_file=avg_n_transition_per_gain_plot)
    plot_gains_n_transitions(aoi_analysis_df, [GAIN_UNDER_THIRTY, GAIN_OF_THIRTY], [purple, XKCD_COLORS["xkcd:darkish purple"]], to_file=avg_n_transition_per_gain_30_plot)
    # plot_gains_n_transitions(aoi_analysis_df, [NEGATIVE_GAIN, POSITIVE_GAIN], [sand, purple], to_file=avg_n_transition_per_gain_plot)
    plot_losses_avg_dwell_time(aoi_analysis_df, [LOSS_UNDER_TEN, LOSS_OF_TEN], is_dwell_a_proportion=dwell_as_a_proportion, colors=[med_blue, blue], to_file=avg_dwell_per_loss_plot)
    ...
    plot_losses_avg_dwell_time(aoi_analysis_df, [LOSS_UNDER_THIRTY, LOSS_OF_THIRTY], is_dwell_a_proportion=dwell_as_a_proportion, colors=[
                               XKCD_COLORS["xkcd:dusk blue"], XKCD_COLORS["xkcd:darkish blue"]], to_file=avg_dwell_per_loss_30_plot)
    plot_losses_n_transitions(aoi_analysis_df, [LOSS_UNDER_THIRTY, LOSS_OF_THIRTY], [XKCD_COLORS["xkcd:dusk blue"], XKCD_COLORS["xkcd:darkish blue"]], to_file=avg_n_transition_per_loss_30_plot)
    ...
    do_lie_percentage_by_condition_chi_squared_tests(aoi_analysis_df, for_test)
    for_test = [(GAIN_OF_TEN, GAIN_UNDER_TEN), (LOSS_OF_TEN, LOSS_UNDER_TEN), (GAIN_UNDER_THIRTY, GAIN_OF_THIRTY), (LOSS_UNDER_THIRTY, LOSS_OF_THIRTY)]
    do_gains_t_test(aoi_analysis_df, for_test).pipe(display, max_cols=None)
    display(get_gains_response_stats(aoi_analysis_df, [LOSS_UNDER_TEN, LOSS_OF_TEN, LOSS_OF_TWENTY, LOSS_OF_THIRTY], val_only=True), max_cols=None, max_rows=None)
...
def pid_dtw_analysis(input_distance_file: str = None,
    ...
    aoi_analysis_df = read_from_analysis_file(input_aoi_analysis_file, [PID, TRIAL_ID, TRIAL_COUNT], dwell_as_a_proportion=dwell_as_a_proportion)
    ...
    display(get_pid_response_stats_for_clusters(cluster_df, aoi_analysis_df, with_error_bounds=False), max_cols=None)
    plot_percent_lies_for_clusters(responses_df, PID, colors, to_file=pid_percent_lies_plot)
    plot_dwell_times_for_clusters(responses_df, PID, dwell_as_a_proportion=dwell_as_a_proportion, colors=colors, to_file=pid_dwell_times_plot)
    ...
    plot_aoi_by_pid(sorted_df, SELF_LIE, dwell_as_a_proportion=dwell_as_a_proportion, colors=colors)
    plot_aoi_by_pid(sorted_df, SELF_TRUE, dwell_as_a_proportion=dwell_as_a_proportion, colors=colors)
    plot_aoi_by_pid(sorted_df, OTHER_LIE, dwell_as_a_proportion=dwell_as_a_proportion,  colors=colors)
    plot_aoi_by_pid(sorted_df, OTHER_TRUTH, dwell_as_a_proportion=dwell_as_a_proportion,  colors=colors)
    ...
    plot_dwell_time_by_gain_for_clusters(gain_under_ten_responses_df, gain_of_ten_responses_df, SELF_LIE,
                                         dwell_as_a_proportion=dwell_as_a_proportion, colors=colors, to_file=self_lie_gain_cluster_plot)
    plot_dwell_time_by_gain_for_clusters(gain_under_ten_responses_df, gain_of_ten_responses_df, SELF_TRUE,
                                         dwell_as_a_proportion=dwell_as_a_proportion, colors=colors, to_file=self_true_gain_cluster_plot)
    plot_dwell_time_by_gain_for_clusters(gain_under_ten_responses_df, gain_of_ten_responses_df, OTHER_LIE,
                                         dwell_as_a_proportion=dwell_as_a_proportion, colors=colors, to_file=other_lie_gain_cluster_plot)
    plot_dwell_time_by_gain_for_clusters(gain_under_ten_responses_df, gain_of_ten_responses_df, OTHER_TRUTH,
                                         dwell_as_a_proportion=dwell_as_a_proportion, colors=colors, to_file=other_truth_gain_cluster_plot)
...
def all_trial_dtw_analysis(input_distance_file: str = None,
                           ...
                           colors: List[str] = XKCD_COLORS_LIST):
    distance_df = read_from_dtw_file(input_distance_file)
    aoi_analysis_df = read_from_analysis_file(input_aoi_analysis_file, index=[PID, TRIAL_ID, TRIAL_COUNT], dwell_in_ms=True)
    big_matrix_df = create_big_matrix(distance_df, filter_by_df=aoi_analysis_df)
    ...
    # plot_n_transitions_by_gain_for_clusters(gain_under_ten_responses_df, gain_of_ten_responses_df, colors, to_file=n_transitions_gain_cluster_plot)
    # plot_dwell_time_by_gain_for_clusters(gain_under_ten_responses_df, gain_of_ten_responses_df, SELF_LIE, colors=colors, to_file=self_lie_gain_cluster_plot)
    # plot_dwell_time_by_gain_for_clusters(gain_under_ten_responses_df, gain_of_ten_responses_df, SELF_TRUE, colors=colors, to_file=self_true_gain_cluster_plot)
    # plot_dwell_time_by_gain_for_clusters(gain_under_ten_responses_df, gain_of_ten_responses_df, OTHER_LIE, colors=colors, to_file=other_lie_gain_cluster_plot)
    ...
    do_heirarchical_logistical_regression('(SELFGAIN + OTHERLOSS) * CLUSTER + (1|PID)', aoi_analysis_df, cluster_df, test=False, plotFolder=HIERARCHICAL_REGRESSION_FOLDER)
    do_heirarchical_logistical_regression('(SELFGAIN + OTHERLOSS) * CLUSTER + (1|SELFGAIN)', aoi_analysis_df, cluster_df, test=False, plotFolder="plots/R_selfgain_random")
