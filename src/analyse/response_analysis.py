...
def plot_n_trials_for_clusters(response_df: DataFrame, cluster_by: str, colors: list[str] = XKCD_COLORS_LIST, title_prefix: str = '', to_file: str = None):
    ...
    plt.ylabel('N Trials')
    plot_response_stats_for_clusters(ax, response_df, [TRIAL_COUNT], CLUSTER, colors, to_file)
...
def plot_n_trials_for_clusters_by_pid(response_df: DataFrame, cluster_by: str, colors: list[str] = XKCD_COLORS_LIST, title_prefix: str = '', to_file: str = None):
    ...
    plt.title('%sQuantity per %s Cluster' % (title_prefix, cluster_by))
    plt.ylabel('N Trials')
    plot_response_stats_for_clusters(ax, response_df, [TRIAL_COUNT], PID, colors, to_file)
...
def plot_response_stats_for_clusters(ax: Axes, responses_df: DataFrame, stats: list[str], index_name: str, colors: list[str] = XKCD_COLORS_LIST, to_file: str = None):
    ...
    if len(stats) > 1 and index_name is CLUSTER:
        ...
        n_bars = len(responses_df.index.get_level_values(PID)) / len(responses_df.index.get_level_values(PID).unique())
        plt.xticks((n_bars * bar_width/2) * (np.array(range(0, len(responses_df.index.get_level_values(PID).unique()))) * 2), responses_df.index.get_level_values(PID).unique(), rotation=0, fontsize=8)
